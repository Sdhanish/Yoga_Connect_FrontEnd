...
const adminNavItems = [
    ...
    { to: "/dashboard/applications", icon: <TbBrandAppleArcade className="text-2xl" />, label: "Applications" },
];
const instructorNavItem = [
    { to: "/dashboard/instructor-cp", icon: <FaHome className="text-2xl" />, label: "Home" },
    { to: "/dashboard/add-class", icon: <MdExplore className="text-2xl" />, label: "Add Classes" },
...
const lastMenuItems = [
    ...
    { to: "/classes", icon: <MdOfflineBolt className="text-2xl" />, label: "Classes" },
    { to: "/", icon: <GiFigurehead className="text-2xl" />, label: "Trending" },
];
            ...
            >
                <div className="flex gap-x-4 items-center">
                    <img
                        ...
                        className={`rounded-full object-cover cursor-pointer h-[40px] duration-500 ${open && "rotate-[360deg]"
                            }`}
                    />
                    <h1
                ...
                {/* Nav links  */}
                {
                    role === 'admin' && <ul className="pt-6">
                        <p className={`ml-3 text-light-gray-4 ${!open && "hidden"}`}><small className='tracking-[8px]'>MENU</small></p>
                        {role === 'admin' && adminNavItems.map((menuItem, index) => (
                            <li key={index} className="mb-2">
                                <NavLink
                                    to={menuItem.to}
                                    className={({ isActive }) =>
                                        `flex ${isActive ? "bg-[#2ac769] text-white " : "text-[#413F44]"
