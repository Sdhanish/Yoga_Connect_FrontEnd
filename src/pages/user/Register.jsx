...
const Register = () => {
  useTitle("Register | YogaConnect Unite with Your True Self.");
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const apiKey = import.meta.env.VITE_IMG_TOKEN; // Use your environment variable for the API key
      // Upload image to ImageBB
      const imgbbResponse = await axios.post(
        `https://api.imgbb.com/1/upload?key=${apiKey}`,
        formData
      );
      const photoUrl = imgbbResponse.data.data.display_url; // Extract URL from ImageBB response
      ...
      // Use toast.promise for the rest of the registration logic
      toast.promise(
        signUp(data.email, data.password)
          .then((userCredential) => {
            ...
            if (user) {
              ...
              return updateUser(data.name, photoUrl).then(() => {
                const userImp = {
                  name: user.displayName,
                  email: user.email,
                  photoUrl: photoUrl, // Use the photoUrl from the upload
                  gender: data.gender,
                ...
                if (user.email && user.displayName) {
                  return axios
                    .post("https://yoga-connect-new-server.onrender.com/new-user", userImp)
                    .then(() => {
                      navigate("/dashboard");
      ...
      <div className="bg-white p-8 rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                {...register("name", {
                  ...
                  pattern: {
                    value: /^[A-Za-z][A-Za-z\s]*$/, // Allows only letters and spaces, no special characters
                    message: "Must start with a letter",
                  },
                  validate: {
                    notOnlyNumbers: (value) =>
                      !/^\d+$/.test(value) || "Name cannot be only numbers",
            ...
            <div className="mb-4">
              ...
              <input
                ...
                {...register("phone", {
                  required: true,
                  pattern: {
                    value: /^(\d{10}|\d{12})$/, // Allows only 10 or 12 digits (local number or country code with local number)
                    message: "Must be 10 or 12 digits long",
                  },
                })}
                className="w-full border border-gray-300 rounded-md py-2 px-4 focus:outline-none focus:ring focus:border-blue-300"
              />
          ...
          <div className="text-center">
            <button
              type="submit"
              className="bg-secondary hover:bg-green-900 text-white py-2 px-4 rounded-md"
            >
              Register
            </button>
            {errors.name && (
